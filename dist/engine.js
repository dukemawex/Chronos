...
exports.threatTier = threatTier;
exports.huntPeriodicActivity = huntPeriodicActivity;
exports.huntIamChanges = huntIamChanges;
exports.huntS3PolicyChanges = huntS3PolicyChanges;
exports.huntBruteForce = huntBruteForce;
exports.huntNewIpIamChain = huntNewIpIamChain;
exports.analyse = analyse;
exports.generateSecurityBrief = generateSecurityBrief;
exports.generateThreatIntelReport = generateThreatIntelReport;
const cloudtrail_1 = require("./parsers/cloudtrail");
// ---------------------------------------------------------------------------
// Scoring
// ---------------------------------------------------------------------------
const SEVERITY_DEFAULT_CONFIDENCE = {
    CRITICAL: 75,
    HIGH: 55,
    MEDIUM: 35,
    LOW: 15,
 ...
 */
function isPeriodic(gaps, tolerance = 0.15) {
    if (gaps.length < 3)
        return { periodic: false, period: 0 };
 ...
 */
function huntPeriodicActivity(events) {
    const findings = [];
    const byIp = groupBy(events, (e) => e.srcIp || "unknown");
    for (const [ip, ipEvents] of byIp) {
        if (ipEvents.length < PERIODIC_MIN_EVENTS)
            continue;
        ...
        if (!periodic || period < PERIODIC_MIN_INTERVAL_SECONDS)
            continue;
        ...
        findings.push({
            ...
            severity: "MEDIUM",
            hypothesis: (`Source IP ${ip} issued ${sorted.length} CloudTrail API call(s) ` +
 ...
 */
function huntIamChanges(events) {
    const findings = [];
    const iamEvents = events.filter((e) => e.source === "cloudtrail" && IAM_SENSITIVE_EVENTS.has(e.eventType));
    if (iamEvents.length === 0)
        return findings;
    ...
    for (const [user, userEvents] of byUser) {
        ...
        const isEscalation = types.includes("CreateUser") &&
        ...
        findings.push({
            ...
            hypothesis: (`User '${user}' performed ${sorted.length} sensitive IAM operation(s): ` +
                `${types.join(", ")}. ` +
                (isEscalation
 ...
 */
function huntS3PolicyChanges(events) {
    const findings = [];
    const s3Events = events.filter((e) => e.source === "cloudtrail" && S3_SENSITIVE_EVENTS.has(e.eventType));
    for (const e of s3Events) {
        const bucket = String(e.requestParameters?.bucketName ?? "unknown");
        findings.push({
            title: `S3 Bucket Policy Change — ${e.eventType} on '${bucket}'`,
            severity: "HIGH",
            hypothesis: (`The S3 bucket '${bucket}' had its policy or ACL modified ` +
 ...
 */
function huntBruteForce(events) {
    const findings = [];
    const failedEvents = events.filter((e) => e.source === "cloudtrail" &&
    ...
    for (const [ip, ipEvents] of byIp) {
        ...
        for (let windowEnd = 0; windowEnd < sorted.length; windowEnd++) {
            while ((sorted[windowEnd].timestamp.getTime() -
            ...
            if (window.length >= BRUTE_FORCE_THRESHOLD) {
                ...
                findings.push({
                    ...
                    severity: "HIGH",
                    hypothesis: (`IP ${ip} made ${window.length} failed authentication attempt(s) ` +
...
function huntNewIpIamChain(events) {
    ...
    for (const [user, userLogins] of byUser) {
        ...
        if (iamFollowups.length === 0)
            continue;
        const evidence = [latestLogin, ...iamFollowups].sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
        findings.push({
            title: `APT — New-IP Login → IAM Change by '${user}'`,
            severity: "CRITICAL",
            confidence: 85,
            hypothesis: (`User '${user}' logged in from a new IP ${latestLogin.srcIp} ` +
 ...
 */
function analyse(inputs, sources) {
    const allEvents = [];
    for (const raw of inputs) {
        allEvents.push(...(0, cloudtrail_1.parseCloudTrail)(raw));
    }
 ...
 */
function generateSecurityBrief(result) {
    const { events, findings, logSources, analysisTime, threatScore, threatTierLabel } = result;
    const lines = [];
    ...
    if (findings.length === 0) {
        ...
        for (const f of findings) {
            lines.push(`- ${severityEmoji(f.severity)} **${f.title}** (confidence: ${f.confidence ?? "—"})`);
        }
    }
    lines.push("");
    // Timeline
    ...
    lines.push("");
    const allTimeline = findings.flatMap((f) => f.timeline.map(([ts, desc]) => [ts, desc, f.title]));
 ...
 *   5. Remediation Plan
 */
function generateThreatIntelReport(result) {
    const { events, findings, logSources, analysisTime, threatScore, threatTierLabel } = result;
    const lines = [];
    ...
    lines.push(`| High Findings | ${findings.filter((f) => f.severity === "HIGH").length} |`);
    ...
    const supportingFindings = findings.filter((f) => !["CRITICAL", "HIGH"].includes(f.severity));
    ...
    for (const f of supportingFindings) {
        lines.push(`- ${severityEmoji(f.severity)} **${f.title}** — ${f.hypothesis.slice(0, 120)}…`);
    }
    lines.push("");
    // Section 4: Unified Attack Timeline
    lines.push("## 4. Unified Attack Timeline");
    lines.push("");
    const allTimeline = findings.flatMap((f) => f.timeline.map(([ts, desc]) => [ts, desc, f.severity]));
    allTimeline.sort((a, b) => a[0].getTime() - b[0].getTime());
    ...
    for (const f of critical) {
        for (const m of f.mitigations.slice(0, 2))
            lines.push(`- ${m}`);
    }
    lines.push("");
    ...
    for (const f of [...critical, ...high]) {
        for (const m of f.mitigations.slice(2))
            lines.push(`- ${m}`);
    }
    lines.push("");
